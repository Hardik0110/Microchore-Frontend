      ...
      <div className="flex flex-col gap-6">
        ...
        <Card className="p-6 flex flex-col gap-2">
          ...
          <p className="text-xs text-ink-3">
            Once granted reviewer status (T1, T2, or ADMIN), refresh this page.
          </p>
        </Card>
      </div>
    )
    ...
    return (
      <div className="flex flex-col gap-6">
        ...
        <Card className="p-6 flex flex-col gap-2">
          ...
          <p className="text-xs text-ink-3">
            Check back in a few minutes as workers submit comments.
          </p>
        </Card>
      </div>
    )
    ...
    <div className="flex flex-col gap-6">
      ...
      <Card className="p-5 flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-1">
          <p className="text-xs text-ink-3 uppercase tracking-wide">Target post</p>
          <a
        ...
        <div className="flex flex-col gap-1">
          <p className="text-xs text-ink-3 uppercase tracking-wide">Keyword required</p>
          <p className="text-sm text-ink font-medium">{current.keyword || '(none)'}</p>
        </div>

        <div className="flex flex-col gap-1">
          <p className="text-xs text-ink-3 uppercase tracking-wide">Comment text</p>
        ...
        <div className="flex flex-col gap-1">
          <p className="text-xs text-ink-3 uppercase tracking-wide">Comment URL</p>
          <a
            href={current.commentUrl || '#'}
            target="_blank"
            rel="noopener noreferrer"
            className="text-sm text-brand hover:underline break-all"
          >
      ...
      <Card className="p-5 flex flex-col gap-4">
        ...
        <button
          ...
          className="self-start rounded-md bg-brand text-white px-4 py-2 text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed hover:bg-brand-deep"
        >
...
function Header({ remaining }: { remaining?: number }) {
  return (
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div>
        <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-ink">Reviewer queue</h1>
        <p className="text-xs text-ink-3 mt-0.5">
          Grade one submission at a time. Three independent reviews finalize each task.
        </p>
      </div>
      {typeof remaining === 'number' && (
        <Link
          to="/app"
          className="text-xs text-ink-2 hover:text-ink underline-offset-4 hover:underline"
        >
          Back to dashboard ({remaining} remaining)
        </Link>
      )}
    </div>
  )
